...
import { scanApi, createScanLogStream } from "../api/client"
...
export default function AutomatedScan({
  ...
  const [currentStage, setCurrentStage] = useState<string>("validate")
  const [findingsLive, setFindingsLive] = useState(FINDINGS_LIVE)
  ...
  // ── WebSocket log stream — replaces fake setInterval ────────────────────
  useEffect(() => {
    if (!scanActive || !activeScanId) return
    ...
    const ws = createScanLogStream(
      ...
      (msg) => {
        ...
        // Animate RPS graph
        setRpsPoints((prev) => {
          const nextVal = Math.floor(30 + Math.random() * 30)
  ...
  }, [activeScanId, scanActive])

  // ── Status polling — updates progress % and stage every 2s ──────────────
  useEffect(() => {
    if (!scanActive || !activeScanId || paused) return

    const poll = setInterval(async () => {
      try {
        const status = await scanApi.status(activeScanId)
        setProgress(status.progress)
        setCurrentStage(status.stage)
        if (status.findings_count > 0) {
          // We have real findings from the backend — keep live panel updated
          setFindingsLive((prev) =>
            prev.length === status.findings_count ? prev : prev
          )
        }
        ...
        // Backend unreachable — fall back to simulated progress
  ...
  }, [scanActive, activeScanId, paused])

  // Fallback: if no scan ID (backend not connected), use simulated progress
  useEffect(() => {
    if (!scanActive || activeScanId || paused) return
    if (progress >= 100) { setScanActive(false); return }
  ...
  const handleStopScan = async () => {
    if (activeScanId) {
      await scanApi.stop(activeScanId).catch(() => {})
    }
    wsRef.current?.close()
    setScanActive(false)
  }
  ...
  const handlePauseScan = async () => {
    if (activeScanId) {
      await scanApi.pause(activeScanId).catch(() => {})
    }
    setPaused(!paused)
  }
