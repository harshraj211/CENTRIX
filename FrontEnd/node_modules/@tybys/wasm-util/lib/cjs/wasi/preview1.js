  ...
  constructor(args, env, fds, asyncFs, fs, asyncify) {
      ...
      function (in_ptr, out_ptr, nsubscriptions, nevents) {
        ...
        for (
          ...
          switch (sub.type) {
            case types_1.WasiEventType.CLOCK: {
              if (
                sub.u.clock.flags ===
                types_1.WasiSubclockflags.ABSTIME
              ) {
                const now =
                  BigInt(Date.now()) *
                  BigInt(1000000)
        ...
        if (has_timeout) {
          ...
          view.setUint32(
            event +
              8,
            types_1.WasiErrno.ESUCCESS,
            true,
          )
          view.setUint32(
            event +
              12,
            types_1.WasiEventType.CLOCK,
            true,
          )
          view.setUint32(nevents, 1, true)
        }
    ...
    function defineImport(
      ...
      if (asyncFs) {
        if (asyncify) {
          ...
          _this[name] = (0, jspi_1.wrapAsyncImport)(
            syscallWrap(_this, name, asyncVersion),
            parameterType,
            returnType,
          )
    ...
    defineImport(
      ...
      function fd_datasync(fd) {
        ...
        const fileDescriptor = wasi.fds.get(
          fd,
          types_1.WasiRights.FD_DATASYNC,
          BigInt(0),
        )
        const fs = getFs(this)
    ...
    function fdFilestatGetTimes(fd, atim, mtim, flags) {
      ...
      if (
        (flags & types_1.WasiFstFlag.SET_ATIM_NOW) ===
        types_1.WasiFstFlag.SET_ATIM_NOW
      ) {
        atim = BigInt(
          Date.now() *
      ...
      if (
        (flags & types_1.WasiFstFlag.SET_MTIM_NOW) ===
        types_1.WasiFstFlag.SET_MTIM_NOW
      ) {
        mtim = BigInt(
          Date.now() *
    ...
    defineImport(
      ...
      function fd_pwrite(fd, iovs, iovslen, offset, size) {
        ...
        const buffer = (0, fd_1.concatBuffer)(
      ...
      function fd_readdir(fd, buf, buf_len, cookie, bufused) {
        ...
        for (
          ...
          const entryInfo = fs.statSync(
            (0, path_1.resolve)(fileDescriptor.realPath, entries[i].name),
            { bigint: true },
          )
    ...
    defineImport(
      ...
      function path_filestat_set_times(
        ...
        if (
          (fst_flags & types_1.WasiFstFlag.SET_ATIM_NOW) ===
          types_1.WasiFstFlag.SET_ATIM_NOW
        ) {
          atim = BigInt(
            Date.now() *
        ...
        if (
          (fst_flags & types_1.WasiFstFlag.SET_MTIM_NOW) ===
          types_1.WasiFstFlag.SET_MTIM_NOW
        ) {
          mtim = BigInt(
            Date.now() *
    ...
    defineImport(
      ...
      function path_link(
        ...
        const resolvedNewPath = (0, path_1.resolve)(
          newWrap.realPath,
          decoder.decode(
            (0, util_1.unsharedSlice)(
              HEAPU8,
    ...
    function pathOpen(o_flags, fs_rights_base, fs_rights_inheriting, fs_flags) {
      const read =
        (fs_rights_base &
          (types_1.WasiRights.FD_READ | types_1.WasiRights.FD_READDIR)) !==
        BigInt(0)
      const write =
        (fs_rights_base &
          (types_1.WasiRights.FD_DATASYNC |
      ...
      let flags = write
        ? read
          ? types_1.FileControlFlag.O_RDWR
          : types_1.FileControlFlag.O_WRONLY
        : types_1.FileControlFlag.O_RDONLY
      ...
      if (
        write &&
        (flags &
          (types_1.FileControlFlag.O_APPEND |
            types_1.FileControlFlag.O_TRUNC)) ===
          0
      ) {
    ...
    defineImport(
      ...
      function path_open(
        ...
        if (
          filetype !==
            types_1.WasiFileType.DIRECTORY &&
          ((o_flags & types_1.WasiFileControlFlag.O_DIRECTORY) !== 0 ||
            resolved_path.endsWith("/"))
        ) {
          return types_1.WasiErrno.ENOTDIR
        }
        const { base: max_base, inheriting: max_inheriting } = (0,
        rights_1.getRights)(wasi.fds.stdio, r, flagsRes, filetype)
      ...
      async function path_open(
        ...
        const { base: max_base, inheriting: max_inheriting } = (0,
        rights_1.getRights)(wasi.fds.stdio, r.fd, flagsRes, filetype)
        const wrap = wasi.fds.insert(
          r,
          resolved_path,
          resolved_path,
    ...
    defineImport(
      ...
      function path_rename(
        ...
        const resolvedOldPath = (0, path_1.resolve)(
          oldWrap.realPath,
          decoder.decode(
            (0, util_1.unsharedSlice)(
              HEAPU8,
    ...
    this._setMemory = function setMemory(m) {
      if (
        !(
          m instanceof
          webassembly_1._WebAssembly.Memory
        )
      ) {
        throw new TypeError(
