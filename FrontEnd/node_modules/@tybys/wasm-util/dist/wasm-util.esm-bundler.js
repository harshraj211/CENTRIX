...
function tryAllocate(instance, wasm64, size, mallocName) {
  ...
  const dataPtr = wasm64
    ? Number(malloc(BigInt(16) + BigInt(size)))
    : malloc(8 + size)
  if (dataPtr === 0) {
...
class Asyncify {
    ...
    if (!options.tryAllocate) {
      ...
      if (options.tryAllocate === true) {
        ...
        address = tryAllocate(
          instance,
          wasm64,
          (_a = options.tryAllocate.size) !== null && _a !== void 0 ? _a : 4096,
          (_b = options.tryAllocate.name) !== null && _b !== void 0
            ? _b
            : "malloc",
        )
      }
  ...
  wrapImportFunction(f) {
    ...
    return function () {
      ...
      while (
        _this.exports.asyncify_get_state() === 2 /* AsyncifyState.REWINDING */
      ) {
  ...
  wrapExportFunction(f) {
    ...
    return async function () {
      ...
      while (
        _this.exports.asyncify_get_state() === 1 /* AsyncifyState.UNWINDING */
      ) {
  ...
  wrapExports(exports, needWrap) {
    return wrapInstanceExports(exports, (exportValue, name) => {
      let ignore =
        ignoreNames.indexOf(name) !== -1 || typeof exportValue !== "function"
...
function asyncifyLoad(asyncify, urlOrBuffer, imports) {
  ...
  return load(urlOrBuffer, imports).then((source) => {
    ...
    const memory =
      ...
      ((_a = imports.env) === null || _a === void 0 ? void 0 : _a.memory)
    return {
...
const PATH_FILESTAT_SET_SIZE =
  /*#__PURE__*/ BigInt(1) << /*#__PURE__*/ BigInt(19)
const PATH_FILESTAT_SET_TIMES =
  /*#__PURE__*/ BigInt(1) << /*#__PURE__*/ BigInt(20)
...
class FileDescriptorTable {
  ...
  insertStdio(fd, expected, name) {
    ...
    const { base, inheriting } = getRights(
      this.stdio,
      fd,
      2 /* FileControlFlag.O_RDWR */,
      type,
    )
  ...
  constructor(args, env, fds, asyncFs, fs, asyncify) {
    this.args_get = syscallWrap(this, "args_get", function (argv, argv_buf) {
      ...
      const { HEAPU8, view } = getMemory(
        this,
        Math.max(argv + args.length * 4, argv_buf + argsSize),
      )
    ...
    this.args_sizes_get = syscallWrap(
      ...
      function (argc, argv_buf_size) {
        ...
        view.setUint32(
          argv_buf_size,
          encoder.encode(args.join("\0") + "\0").length,
          true,
        )
        return 0 /* WasiErrno.ESUCCESS */
      },
    ...
    this.environ_get = syscallWrap(
      ...
      function (environ, environ_buf) {
        ...
        const { HEAPU8, view } = getMemory(
          this,
          Math.max(environ + env.length * 4, environ_buf + envSize),
        )
    ...
    this.environ_sizes_get = syscallWrap(
      ...
      function (len, buflen) {
        ...
        view.setUint32(
          buflen,
          encoder.encode(wasi.env.join("\0") + "\0").length,
          true,
        )
        return 0 /* WasiErrno.ESUCCESS */
      },
        ...
        ? syscallWrap(this, "random_get", function (buf, buf_len) {
            ...
            for (pos = 0; pos + stride < buf_len; pos += stride) {
              crypto.getRandomValues(
                HEAPU8.subarray(buf + pos, buf + pos + stride),
              )
            }
    ...
    function fdFilestatGetTimes(fd, atim, mtim, flags) {
      ...
      if (
        (flags & 2) /* WasiFstFlag.SET_ATIM_NOW */ ===
        2 /* WasiFstFlag.SET_ATIM_NOW */
      ) {
        atim = BigInt(Date.now() * 1000000)
      }
      if (
        (flags & 8) /* WasiFstFlag.SET_MTIM_NOW */ ===
    ...
    defineImport(
      ...
      function fd_pread(fd, iovs, iovslen, offset, size) {
        ...
        const { HEAPU8, view } = getMemory(
          this,
          Math.max(iovs + Number(iovslen) * 8, size + 4),
        )
        const wasi = _wasi.get(this)
        const fileDescriptor = wasi.fds.get(
          fd,
    ...
    defineImport(
      ...
      function fd_readdir(fd, buf, buf_len, cookie, bufused) {
        ...
        const { HEAPU8, view } = getMemory(
          this,
          Math.max(buf + buf_len, bufused + 4),
        )
        let bufferUsed = 0
    ...
    defineImport(
      ...
      function path_filestat_get(fd, flags, path, path_len, filestat) {
        ...
        const { HEAPU8, view } = getMemory(
          this,
          Math.max(path + path_len, filestat + 64),
        )
        const wasi = _wasi.get(this)
        const fileDescriptor = wasi.fds.get(
          fd,
    ...
    defineImport(
      ...
      function path_filestat_set_times(
        ...
        if (
          (fst_flags & 2) /* WasiFstFlag.SET_ATIM_NOW */ ===
          2 /* WasiFstFlag.SET_ATIM_NOW */
        ) {
          atim = BigInt(Date.now() * 1000000)
        }
        if (
          (fst_flags & 8) /* WasiFstFlag.SET_MTIM_NOW */ ===
          8 /* WasiFstFlag.SET_MTIM_NOW */
        ) {
          mtim = BigInt(Date.now() * 1000000)
        }
    ...
    function pathOpen(o_flags, fs_rights_base, fs_rights_inheriting, fs_flags) {
      ...
      let flags = write
        ? read
          ? 2 /* FileControlFlag.O_RDWR */
          : 1 /* FileControlFlag.O_WRONLY */
        : 0 /* FileControlFlag.O_RDONLY */
    ...
    defineImport(
      ...
      function path_open(
        ...
        if (
          filetype !== 3 /* WasiFileType.DIRECTORY */ &&
...
// eslint-disable-next-line spaced-comment
const kEmptyObject = /*#__PURE__*/ Object.freeze(
...
function validateOptions(options) {
  ...
  const args = ((_a = options.args) !== null && _a !== void 0 ? _a : []).map(
    String,
  )
  const env = []
