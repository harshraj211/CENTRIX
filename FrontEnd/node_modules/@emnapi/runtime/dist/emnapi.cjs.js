...
class NotSupportWeakRefError extends EmnapiError {
  constructor(api, message) {
    super(
      `${api}: The current runtime does not support "FinalizationRegistry" and "WeakRef".${
        message ? ` ${message}` : ""
      }`,
    )
  }
...
class NotSupportBufferError extends EmnapiError {
  constructor(api, message) {
    super(
      `${api}: The current runtime does not support "Buffer". Consider using buffer polyfill to make sure \`globalThis.Buffer\` is defined.${
        message ? ` ${message}` : ""
      }`,
    )
  }
  ...
  adjust(changeInBytes) {
    ...
    if (
      ...
      throw new RangeError(
        `changeInBytes ${changeInBytes} is out of reasonable range`,
      )
    }
