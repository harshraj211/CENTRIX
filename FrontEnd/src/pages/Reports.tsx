...
export default function Reports({ onNavigate, findingsCount }: ReportsProps) {
  ...
  // Load list of reports from backend on mount
  useEffect(() => {
    reportsApi.list().then((reports) => {
      if (reports.length > 0) {
        ...
        setReportsList((prev) => [...mapped, ...prev])
      }
    }).catch(() => {})

    // Also grab the most recent scan ID from backend
    import("../api/client").then(({ scanApi }) => {
      scanApi.list().then((scans) => {
        if (scans.length > 0) {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      if (activeScanId) {
        // Real backend report generation
        ...
        setReportsList([mapped, ...reportsList])
      ...
      const newReport = {
        id: `RPT-00${Math.floor(48 + Math.random() * 50)}`,
        name: `${reportType.charAt(0).toUpperCase() + reportType.slice(1)} Report — ${targetScope}`,
        type: reportType.charAt(0).toUpperCase() + reportType.slice(1),
        target: targetScope,
        generated: "Just now",
        status: "Ready",
